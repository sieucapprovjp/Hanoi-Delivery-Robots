...
class Profiler:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            cls._instance.log_file = os.path.join(
                cls._instance.log_dir, "performance.log"
            )
    ...
    def save_to_log(self):
        ...
        with open(self.log_file, "w") as f:
            ...
            f.write(
                f"{'Label':<40} | {'Count':<8} | {'Avg (ms)':<10} | {'Max (ms)':<10}\n"
            )
            f.write("-" * 75 + "\n")
            for label, data in sorted(self.stats.items()):
                f.write(
                    f"{label:<40} | {data['count']:<8} | {data['avg_time'] * 1000:10.4f} | {data['max_time'] * 1000:10.4f}\n"
                )
            f.write("=" * 75 + "\n")
