...
def build_metrics_payload(metrics, graph, rain_count, traffic_count, obstacle_count):
    avg_path = sum(metrics["pathLengths"]) / len(metrics["pathLengths"]) if metrics["pathLengths"] else 0
    return {
        "pathfinding": {
            ...
            "minCalculationTime": round(metrics["minCalculationTime"], 2) if metrics["minCalculationTime"] < 999 else 0,
