    ...
    return `
        <div class="robot-popup">
            ...
            <div class="popup-grid-3">
                <div class="grid-item"><div class="item-label">Speed</div><div class="item-value">${(robot.speedMultiplier * 100).toFixed(0)}%</div></div>
                <div class="grid-item"><div class="item-label">Battery</div><div class="item-value ${batteryClass}">${robot.battery.toFixed(1)}%</div></div>
                <div class="grid-item"><div class="item-label">Drain</div><div class="item-value">${robot.batteryDrain.toFixed(3)}/s</div></div>
            </div>

            <div class="popup-grid-2">
                <div class="grid-item"><div class="item-label">Completed</div><div class="item-value color-success">${robot.totalDeliveries}</div></div>
                <div class="grid-item"><div class="item-label">Distance</div><div class="item-value color-primary">${robot.totalDistance.toFixed(0)}m</div></div>
            </div>

            ${robot.routeTarget ? `<div class="popup-info-box status-success">🎯 ${robot.routeTarget.lat.toFixed(4)}, ${robot.routeTarget.lon.toFixed(4)}<br>${robot.currentPath.length - robot.pathIndex} waypoints left<br>⏱ ETA ${robot.getEtaText()}</div>` : ''}

            ${robot.currentDelivery ? `<div class="popup-info-box status-neutral"><div class="item-label">📦 Order #${robot.currentDelivery.id}</div><div class="item-value">${robot.deliveryPhase === CONFIG.ROBOT.PHASES.TO_PICKUP ? '🔵 Going to pickup' : '🔴 Going to deliver'}</div></div>` : '<div class="popup-info-box status-neutral">No delivery</div>'}
        </div>
    `;
}
...
function renderRobotRouteInfo(robot) {
    if (!robot.lastRouteBreakdown) {
        return '<div class="insight-box text-center">Waiting for route calculation...</div>';
    }
    ...
    return `
        <div class="insight-box">
            ...
            <div class="breakdown-container">
                <div class="breakdown-row"><span>📏 Base Distance:</span><strong>${robot.lastRouteBreakdown.baseDistance.toFixed(0)}m</strong></div>
                <div class="breakdown-row"><span>🚗 Traffic Penalty:</span><span class="color-error">+${robot.lastRouteBreakdown.trafficPenalty.toFixed(0)}m</span></div>
                <div class="breakdown-row"><span>🌧️ Rain Penalty:</span><span class="color-primary">+${robot.lastRouteBreakdown.rainPenalty.toFixed(0)}m</span></div>
                <div class="breakdown-row"><span>🚧 Obstacle Penalty:</span><span class="color-warning">+${robot.lastRouteBreakdown.obstaclePenalty.toFixed(0)}m</span></div>
                <div class="breakdown-total"><span>🎯 Total Cost:</span><strong>${robot.lastRouteBreakdown.totalCost.toFixed(0)}m</strong></div>
                <div class="breakdown-row"><span>⏱ Sim ETA:</span><strong class="color-success">${robot.getEtaText()}</strong></div>
            </div>
        </div>
    `;
}
    ...
    return `
        <div class="computing-details">
            <div class="details-header">
                🤖 ${robot.name} <span>| Computing Engine</span>
            </div>
            ...
            <div class="stats-grid-3">
                <div class="stat-card"><div class="stat-label">Speed</div><div class="stat-value">${(robot.speedMultiplier * 100).toFixed(0)}%</div></div>
                <div class="stat-card"><div class="stat-label">Battery</div><div class="stat-value ${batteryClass}">${robot.battery.toFixed(1)}%</div></div>
            ...
            <div class="performance-container">
                ...
                <div class="perf-grid">
                    <div class="perf-card"><div class="stat-label">Deliveries</div><div class="stat-value text-success">${robot.totalDeliveries}</div></div>
                    <div class="perf-card"><div class="stat-label">Distance</div><div class="stat-value text-info">${robot.totalDistance.toFixed(0)}m</div></div>
                    <div class="perf-card"><div class="stat-label">Calculations</div><div class="stat-value text-highlight">${robot._calcHistory?.length || 0}</div></div>
                </div>
